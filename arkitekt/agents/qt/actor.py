...
class QtOldActor(QObject):
    ...
    def set_assign_future_qtloop(self, reference, returns):
        if len(returns) == 1:
            self.loop.call_soon_threadsafe(
                self.futureMap[reference].set_result, returns[0]
            )
        if len(returns) >= 2:
            self.loop.call_soon_threadsafe(
                self.futureMap[reference].set_result, returns
            )
    ...
    def set_error_future_qtloop(self, reference, exception):
        self.loop.call_soon_threadsafe(
            self.futureMap[reference].set_exception, exception
        )

    async def on_provide(self, message):
        if not self.qt_on_provide:
            return None
