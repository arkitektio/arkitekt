...
def build_qtwidget_for_argport(port: ArgPort):
    qtwidgetClass = get_widget_registry().get_widget_class(
        port.typename, port.widget.typename
    )
...
class AssignWidget(QtWidgets.QDialog):
    def __init__(
        self, *args, node: Node = None, set_args=[], set_kwargs={}, **kwargs
    ) -> None:
        ...
        assert len(self.set_args) <= len(
            self.node.args
        ), "Was provided more Args than node accepts"
    ...
    def build_args_widget(self):
        ...
        for arg, port in zip_longest(self.set_args, self.node.args):
            if arg:
                ...
                self.argsLayout.addRow(
                    QtWidgets.QLabel(parent=self, text=port.label or port.key), widget
                )
    ...
    def build_ui(self):
        ...
        self.setWindowTitle(
            f"Assigning to {self.node.name} : {self.node.package}/{self.node.interface}"
        )
