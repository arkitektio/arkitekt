...
def is_entrypoint_change(changes: MutableSet[Tuple[Any,str]], entrypoint_real_path: str) -> bool:
...
async def dev_module(identifier, version, entrypoint,  builder: str = "arkitekt.builders.easy", deep: bool = False):
    ...
    panel = Panel(generation_message, style="bold green", border_style="green", title="Arkitekt Dev Mode")
    console.print(panel)
    ...
    try:
        ...
        app: Arkitekt = builder_func(identifier, version)
        ...
        console.print_exception()
        panel = Panel("Error building initial App", style="bold red", border_style="red")
        console.print(panel)
    ...
    async for changes in awatch(".", watch_filter=EntrypointFilter(entrypoint) if not deep else DeepFilter(), debounce=2000, step=500):
        ...
        if not x or x.done():
            ...
            x.cancel()
            panel = Panel("Cancelling latest version", style="bold yellow", border_style="yellow")
            console.print(panel)
            try:
                await x
        ...
        try:
            with console.status("Reloading module..."):
                if is_entrypoint_change(changes, entypoint_real_path):
                    panel = Panel("Detected Entrypoint change, resetting app", style="bold cyan", border_style="cyan")
                    console.print(panel)
                    reset_structure()
        ...
        try:
            ...
            console.print_exception()
            panel = Panel("Error building reloaded App", style="bold red", border_style="red")
            console.print(panel)
