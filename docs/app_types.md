...
*   **Execution:** You don't run these apps with `python app.py` in production. Instead, you use the CLI commands:
    *   `arkitekt run dev`: Runs the app in development mode (hot-reloading, local connection).
    *   `arkitekt run prod`: Runs the app in production mode (simulating the container environment).
