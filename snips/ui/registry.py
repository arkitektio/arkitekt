...
class WidgetRegistry:
    ...
    def get_widget_class(self, portType, widgetType):
        portType = (
            portType.__class__.__name__ if isinstance(portType, Port) else portType
        )
        widgetType = (
            widgetType.typename if isinstance(widgetType, Widget) else widgetType
        )
        try:
            return self.portTypeWidgetTypeQTWidgetMap[portType][widgetType]
        except KeyError as e:
            raise NoWidgetRegisteredError(
                f"No widget registered for {portType} and {widgetType}"
            ) from e
    ...
    def register(self, list: List):
        def register(widget_class):
            for pair in list:
                ...
                try:
                    portType = (
                        portType.__name__ if issubclass(portType, Port) else portType
                    )
                except:
                    portType = portType
                try:
                    widgetType = (
                        widgetType.__name__
                        if issubclass(widgetType, Widget)
                        else widgetType
                    )
                except:
                    widgetType = widgetType
                self.portTypeWidgetTypeQTWidgetMap.setdefault(portType, {})[
                    widgetType
                ] = widget_class
...
def get_widget_registry(register_defaults=True):
    ...
    if WIDGET_REGISTRY is None:
        ...
        if register_defaults:
            WIDGET_REGISTRY.register_defaults()
